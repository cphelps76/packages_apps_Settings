...
public class EthInfo extends PreferenceActivity {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        addPreferencesFromResource(R.xml.device_info_ethinfo);
   	...
   	private String getEthMacfromEfuse() {
				...
				sw.writeSysfs("/sys/class/aml_keys/aml_keys/version", "nand3"); 
				sw.writeSysfs("/sys/class/aml_keys/aml_keys/key_name", "mac"); 
				//sn = sw.readSysfs("/sys/class/aml_keys/aml_keys/key_read");
				//Log.d(TAG, "=========/sys/class/efuse/mac: " + sn);
				//return sn;
				try {
					BufferedReader reader = new BufferedReader(new FileReader("/sys/class/efuse/mac"), 12);
					try {
						sn = reader.readLine();
						}
					finally{
